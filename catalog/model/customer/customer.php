...
class ModelCustomerCustomer extends Model
{
	...
	{
		// echo "UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int) $data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', telephone = '" . $this->db->escape($data['telephone']) . "' WHERE customer_id = '" . (int) $customer_id . "'";
		// die;
		...
		// die;
		// echo "INSERT INTO " . DB_PREFIX . "customer_to_customer_group SET customer_group_id ='" . (int) $data['customer_group_id'] . "', customer_id ='" . (int) $customer_id . "'";
		// die;
		// $this->db->query("INSERT INTO " . DB_PREFIX . "customer_to_customer_group SET customer_group_id ='" . (int) $data['customer_group_id'] . "', store_id ='" . (int) ['store_id'] . "' AND customer_id='" . (int) $customer_id . "'");
		// $this->db->query("INSERT INTO " . `oc_customer_to_customer_group` . "customer SET customer_group_id = '" . (int) $customer_group_id . "customer WHERE customer_id = '" . (int) $customer_id . "'");
		return $customer_id;
	}
	public function updateCustomerToGroup($customer_group_id, $data)
	{
		// echo "UPDATE " . DB_PREFIX . "customer_to_customer_group SET customer_id = '" . (int) $data['customer_id'] . "' WHERE customer_group_id = '" . (int) $customer_group_id . "'";
		// die;
		$this->db->query("UPDATE " . DB_PREFIX . "customer_to_customer_group SET customer_id = '" . (int) $data['customer_id'] . "' WHERE customer_group_id = '" . (int) $data['customer_group_id'] . "'");
	}

	public function updateTableData($customer_id, $data)
	{	
		$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int) $customer_id . "'");
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "cart SET type = '" . $data['type'] . "', table_number = '" . $this->db->escape($data['tableNumber'])	 . "', service_charge_type = '" . $data['serviceChargeType']	 . "', comment = '" . $this->db->escape($data['comments']) . "', customer_id = '" . (int) $customer_id . "', date_added = NOW()");
		
		
	}
	public function getTableData($customer_id)
	{
		// echo "SELECT  customer_id,type,table_number,comment FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int) $customer_id . "'";
		// die;
		$query = $this->db->query("SELECT  customer_id,type,table_number,service_charge_type,comment FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int) $customer_id . "'");
	...
	public function getPersons()
	{
		...
		$query = $this->db->query("SELECT oc_customer.*,oc_store.name FROM oc_customer JOIN oc_store ON oc_customer.store_id = oc_store.store_id WHERE customer_id NOT IN (select customer_id FROM oc_subscriptions)");

		//$query = $this->db->query("SELECT * from oc_customer where store_id=0");
		return $query->rows;
	}
	...
	public function getCustomersFromGroup($data, $limit, $page, $customer_group_id, $exclude, $store_id)
	{
		if ($exclude == 0) {
			$sql = "SELECT customer_id,customer_group_id,store_id,firstname,lastname,CONCAT(firstname, ' ', lastname) AS name ,email,telephone,fax,password,salt,cart,wishlist,newsletter,address_id,custom_field,ip,status,safe,token,code,date_added  FROM " . DB_PREFIX . "customer WHERE customer_id  IN (SELECT customer_id FROM oc_customer_to_customer_group WHERE customer_group_id = '" . (int) $customer_group_id . "')";
		} else {
			$sql = "SELECT customer_id,customer_group_id,store_id,firstname,lastname,CONCAT(firstname, ' ', lastname) AS name ,email,telephone,fax,password,salt,cart,wishlist,newsletter,address_id,custom_field,ip,status,safe,token,code,date_added  FROM " . DB_PREFIX . "customer WHERE customer_id  NOT IN (SELECT customer_id FROM oc_customer_to_customer_group WHERE customer_group_id = '" . (int) $customer_group_id . "') AND store_id = " . (int) $store_id;
		}
		if (!empty($data['filter_name'])) {
			$sql .= " AND CONCAT(firstname, ' ', lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}

		$sql .= " LIMIT " . (int) $page . "," . (int) $limit;
	...
	{
		$sql = "SELECT *, firstname,lastname,CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id)";
	...
	{
		$sql = "SELECT count(1) as total, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id)";
	...
	public function getTotalCustomersByCustomerGroupId($data, $customer_group_id, $exclude, $store_id)
	{
		if ($exclude == 0) {
			$sql = "SELECT count(1) AS total FROM " . DB_PREFIX . "customer WHERE customer_id  IN (SELECT customer_id FROM oc_customer_to_customer_group WHERE customer_group_id = '" . (int) $customer_group_id . "')";
		} else {
			$sql = "SELECT count(1) AS total FROM " . DB_PREFIX . "customer WHERE customer_id  Not IN (SELECT customer_id FROM oc_customer_to_customer_group WHERE customer_group_id = '" . (int) $customer_group_id . "') AND store_id = " . (int) $store_id;
		}
		if (!empty($data['filter_name'])) {
			$sql .= " AND CONCAT(firstname, ' ', lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
	...
	public function AddCustomerToGroup($customer_group_id, $customer_id)
	{
		$query = $this->db->query("INSERT INTO " . `oc_customer_to_customer_group` . "customer SET customer_group_id = '" . (int) $customer_group_id . "customer WHERE customer_id = '" . (int) $customer_id . "'");

		return $customer_group_id;
	}
	public function getCustomerGroupById($customer_group_id)
	{


		$query = $this->db->query("SELECT * FROM `oc_customer_group` ");
