...
class ControllerSaleOrder extends Controller
{
	...
	protected function getList()
	{
		...
		if (isset($this->request->get['filter_service_charge'])) {
			$filter_service_charge = $this->request->get['filter_service_charge'];
		} else {
			$filter_service_charge = '';
		}
		if (isset($this->request->get['filter_tax'])) {
			$filter_tax = $this->request->get['filter_tax'];
		} else {
			$filter_tax = '';
		}
		if (isset($this->request->get['filter_sub_total'])) {
			$filter_sub_total = $this->request->get['filter_sub_total'];
		} else {
			$filter_sub_total = '';
		}
		...
		if (isset($this->request->get['filter_service_charge'])) {
			$url .= '&filter_service_charge=' . $this->request->get['filter_service_charge'];
		}
		if (isset($this->request->get['filter_tax'])) {
			$url .= '&filter_tax=' . $this->request->get['filter_tax'];
		}
		if (isset($this->request->get['filter_sub_total'])) {
			$url .= '&filter_sub_total=' . $this->request->get['filter_sub_total'];
		}
		if (isset($this->request->get['filter_store_name'])) {
		...
		$filter_data = array(
			...
			'filter_service_charge' => $filter_service_charge,
		...
		foreach ($results as $result) {
			$order_total_tax = $this->model_sale_order->getOrderTotals1($result['order_id']);
			$order_sub_total = $this->model_sale_order->getOrderTotals2($result['order_id']);
			...
			$data['orders'][] = array(
				...
				'tax' => $order_total_tax['value'],
				'sub_total' => $order_sub_total['value'],
		...
		$data['sort_service_charge'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.service_charge' . $url, true);
		$data['sort_tax'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.tax' . $url, true);
		$data['sort_sub_total'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.sub_total' . $url, true);
		...
		$data['filter_service_charge'] = $filter_service_charge;
		$data['filter_tax'] = $filter_tax;
	...
	public function info()
	{
		...
		if ($order_info) {
			...
			// die;
			$data['date_added'] = date($this->language->get('date_format_short'), strtotime($order_info['date_added'])) . ' ' . date($this->language->get('time_format'), strtotime($order_info['date_added']));
			// print_r(date($this->language->get('date_format_short'), strtotime($order_info['date_added'])) . ' ' . date($this->language->get('time_format'), strtotime($order_info['date_added'])));
			// die;
			$data['firstname'] = $order_info['firstname'];
			$data['lastname'] = $order_info['lastname'];
